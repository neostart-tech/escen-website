<template>
  <div>
    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
      ...
      <div class="bg-[#202a50] text-white py-2 px-4">
          ...
          <div class="flex flex-col md:flex-row justify-between items-center space-y-2 md:space-y-0">
            <div class="flex items-center space-x-4 text-sm">
              <div class="flex items-center space-x-1">
                ...
                <span>contact@ecole-exemple.fr</span>
              </div>
            ...
            <div class="flex items-center space-x-4 text-sm">
              ...
              <div class="hidden md:flex items-center space-x-2">
                <button class="hover:text-[#01b4d5] transition-colors">Espace Parent</button>
                <span class="text-gray-400">|</span>
                <button class="hover:text-[#01b4d5] transition-colors">Espace Élève</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center">
              <NuxtLink to="/" class="flex items-center hover:opacity-90 transition-opacity">
                <img 
                  ...
                  class="h-40 w-auto object-contain lg:h-24"
                >
              </NuxtLink>
            </div>
            ...
            <div class="hidden lg:flex items-center space-x-8">
              ...
              <template v-for="item in navigation" :key="item.name">
                ...
                <div v-if="item.submenu" class="relative group">
                  <button class="text-[#202a50] font-medium hover:text-[#01b4d5] transition-colors flex items-center space-x-1">
                    <span>{{ item.name }}</span>
                    <svg class="w-4 h-4 transition-transform group-hover:rotate-180" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M7 10l5 5 5-5z"/>
                    </svg>
                  </button>
                  
                  <!-- Sous-menu -->
                  <div class="absolute top-full left-0 mt-2 w-64 bg-white shadow-lg rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 border border-gray-100">
                    <div class="py-2">
                      <NuxtLink 
                        v-for="subItem in item.submenu"
                        :key="subItem.name"
                        :to="subItem.href"
                        class="block px-4 py-3 text-sm text-[#202a50] hover:bg-[#01b4d5] hover:text-white transition-colors border-b border-gray-100 last:border-b-0"
                      >
                        {{ subItem.name }}
                      </NuxtLink>
                    </div>
                ...
                <NuxtLink 
                  v-else
                  :to="item.href"
                  class="text-[#202a50] font-medium hover:text-[#01b4d5] transition-colors relative group"
                >
                  {{ item.name }}
              ...
              <NuxtLink 
                to="/inscription" 
                class="bg-gradient-to-r from-[#01b4d5] to-[#0199b8] text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition-all duration-300 shadow-md flex items-center space-x-2 group" 
                style="color: white !important;font-weight: 900;"
              >
            ...
            <div class="lg:hidden">
              <button 
                ...
                class="p-2 rounded-lg text-[#202a50] hover:bg-gray-100 transition-colors"
              >
                <svg 
    ...
    >
      <div class="p-6 h-full overflow-y-auto">
        <!-- Header du menu mobile avec logo TRÈS agrandi -->
          ...
          <template v-for="item in navigation" :key="item.name">
            ...
            <div v-if="item.submenu" class="space-y-2">
              <button 
                ...
                class="w-full flex items-center justify-between py-3 px-4 text-[#202a50] font-medium hover:bg-[#01b4d5] hover:text-white rounded-lg transition-colors"
              >
                <span>{{ item.name }}</span>
                <svg 
              ...
              <div v-if="openSubmenus[item.name]" class="ml-4 space-y-1 border-l-2 border-[#01b4d5] pl-4">
                <NuxtLink
                  ...
                  :to="subItem.href"
                  @click="isMobileMenuOpen = false"
                  class="block py-2 px-4 text-sm text-[#202a50] hover:bg-[#01b4d5] hover:text-white rounded-lg transition-colors"
                >
                  {{ subItem.name }}
                </NuxtLink>
              </div>
            ...
            <NuxtLink
              ...
              :to="item.href"
              @click="isMobileMenuOpen = false"
              class="block py-3 px-4 text-[#202a50] font-medium hover:bg-[#01b4d5] hover:text-white rounded-lg transition-colors"
            >
              {{ item.name }}
            </NuxtLink>
          </template>
        ...
        <div class="mt-8 space-y-4">
          <NuxtLink
            to="/inscription"
            @click="isMobileMenuOpen = false"
            class="flex items-center justify-center space-x-2 w-full bg-gradient-to-r from-[#01b4d5] to-[#0199b8] text-white text-center py-3 px-4 rounded-lg font-semibold hover:shadow-lg transition-all"
          >
